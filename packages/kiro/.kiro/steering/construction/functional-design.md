...
| `code-flow.md` | **Mermaid `sequenceDiagram` of the function-call path for each user action in this unit** (use `.kiro/templates/03-construction/design/code-flow.md`). Derives from `user-flows.md` (Stage 7) + business-logic-model + `components.md`. Declares cross-boundary calls (each backed by an ADR). Stage 14c sub-check #5 audits code's actual call paths against this. |
