...
Write to aidlc-docs/inception/product-design/. Produce FIVE outputs in order (v0.7 + v0.8):
  1. **`design-tokens.md`** (use templates/02-inception-design/ui-ux/design-tokens.md) — project-level catalog: inherit from KAFI base + declare overrides + WCAG contrast notes. Tokens for colors, typography, spacing, radius, shadow, motion, z-index, breakpoints + component library decision (shadcn/ui · custom · …).
  2. **`uiux-spec.md`** (use templates/02-inception-design/ui-ux/uiux-spec.md) — master narrative: sitemap, navigation chrome, screen catalog, cross-screen flows, key UX decisions, accessibility posture, story → screen coverage matrix.
