---
name: kafi-aidlc-onboarding
description: KAFI AI-DLC onboarding guide. Use when a user has just unzipped or installed the AI-DLC package, asks "how do I start", "what stage am I at", "where am I in AI-DLC", mentions running /init, or wants to scan files in 00-knowledge/ to classify which AI-DLC stage their project is at. Reads 00-knowledge/ and aidlc-docs/ to determine project state across the 17-stage workflow.
---
...
Per Claude Code's documented behavior, running `/init` on a project that already has `CLAUDE.md` does **not** overwrite the file. It scans the codebase and *suggests* updates which the user can selectively merge. The design is complementary, not destructive.

That said, on an AI-DLC project `/init` is **unnecessary**:

- `CLAUDE.md` already contains the complete 17-stage workflow spec.
- Claude Code auto-loads `CLAUDE.md` on every session start — no init step needed.
...
1. Explain it's redundant on an AI-DLC project — `CLAUDE.md` is already loaded.
...
4. If `CLAUDE.md` was modified in a way that diluted the workflow, restore from the original zip or `git checkout` the file.
...
Tell user:
> "Setup complete. Close this skill and start a fresh prompt. The agent will load `CLAUDE.md` automatically, run Stage 1 (Workspace Detection), and route you to the right starting point. No `/init` needed — `CLAUDE.md` is already loaded."
...
When Mode B detects a stage, load the corresponding prompt template from `prompts/` and paste it **verbatim** into the agent session start (replacing placeholders with detected context).

| Detected position | Prompt template file |
|---|---|
| Pre-Inception D (author-from-brief), Stage 4 (Requirements), Stage 6 (Workflow Planning) | `prompts/pm-stage-4-requirements.md` |
| Pre-Inception B/C (fill-gaps / map-existing), Stage 5 (User Stories) | `prompts/ba-stage-5-stories.md` |
| Stage 3 (Reverse Engineering), Stage 8 (Application Design), Stage 9 (Units Generation), Stage 10 (Functional Design), Stage 11 (NFR Requirements), Stage 12 (NFR Design) | `prompts/sa-stage-8-app-design.md` |
| Stage 7 (Product Design) | `prompts/designer-stage-7-product-design.md` |
| **Stage 10b (Unit Test Planning · QA · NEW v0.7)** | `prompts/qa-stage-10b-test-plan.md` |
| Stage 14a (Production Code Generation), Stage 15 (Build) | `prompts/dev-stage-14-code-gen.md` |
| **Stage 14b (Unit Test Code Generation · Dev · NEW v0.7)** | `prompts/dev-stage-14b-unit-tests.md` |
| **Stage 14c (Conformance Audit · QA · NEW v0.7 · BLOCKING)** | `prompts/qa-stage-14c-conformance-audit.md` |
| Stage 13 (Infrastructure Design) | `prompts/devops-stage-13-infra-design.md` |
...
Agent has already loaded the matching prompt template from `prompts/<file>.md` and substituted placeholders. The session is ready to start — no manual paste needed.

If user wants to inspect / customize the prompt before starting, the verbatim content is:
```
[Full prompt body from prompts/<role>.md with placeholders substituted]
```
...
Template selected: `prompts/ba-stage-5-stories.md` (Pre-Inception C requires BA-driven Vision mapping). Placeholders substituted from detected context. Session ready.
...
1. **`/init` is redundant on AI-DLC projects** — it's complementary (suggests, doesn't overwrite per Claude Code design), but `CLAUDE.md` is already loaded. Encourage user to use this skill's stage detection instead.
...
- `aidlc-rule-details/inception/workspace-detection.md` — Stage 1 detection logic (this skill front-loads it)
- `aidlc-rule-details/common/process-overview.md` — 10-step cycle including state file maintenance
- `aidlc-rule-details/common/session-continuity.md` — resume protocol
